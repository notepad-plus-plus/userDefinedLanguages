...
$testRoot = ".\functionList\"
$PowerEditorLocal = ".\PowerEditor"

$dirName=$args[0]
$langName=$args[1]

Write-Debug "unitTest: root=$testRoot local=$PowerEditorLocal dir=$dirName lang=$langName"
...
Try {
	if ((Get-Item $testRoot$dirName) -is [System.IO.DirectoryInfo])
	{
		...
		if ($langName.StartsWith("udl-"))
		{
			...
			Write-Debug ":: $PowerEditorLocal\bin\notepad++.exe -multiInst -nosession -export=functionList -udl='$langName' $testRoot$dirName\unitTest | Out-Null"
			& "$PowerEditorLocal\bin\notepad++.exe" -multiInst -nosession -export=functionList -udl="$langName" $testRoot$dirName\unitTest | Out-Null
			& "$PowerEditorLocal\bin\notepad++.exe" -multiInst -nosession                      -udl="$langName" $testRoot$dirName\unitTest | Out-Null
		}
		else
		{
			Write-Debug "$PowerEditorLocal\bin\notepad++.exe -multiInst -nosession -export=functionList -l'$langName' $testRoot$dirName\unitTest | Out-Null"
			& "$PowerEditorLocal\bin\notepad++.exe" -multiInst -nosession -export=functionList -l"$langName" $testRoot$dirName\unitTest | Out-Null
		}
